    ...
    public class DataMessageBLL : NotifyPropertyChangeObject
    {
        ...
        public bool SplitCoil(int coilExtension)
        {
            try
            {
                ...
                {
                    int coilExtensionMax = SQLDatabase.GetScalarValue("SELECT MAX(CoilExtension) AS CoilExtensionMax FROM DataMessageMaster WHERE CoilID = " + this.DataMessageMaster.CoilID);
                    ...
                    if (coilExtensionMax == 0 && SQLDatabase.ExecuteNonQuery("UPDATE DataMessageMaster SET CoilExtension = 1 WHERE DataMessageID = " + this.DataMessageMaster.DataMessageID) != 1) throw new Exception("Lỗi cập nhật cuộn 00 -> 01.");
